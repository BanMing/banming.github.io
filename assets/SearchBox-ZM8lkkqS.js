import{g as f,h as C,i as D,j as R,t as M,u as L,k as N,o as u,c as m,l as F,v as U,e as y,n as b,m as w,F as G,p as I,a as E,q as v,s as B}from"./app-M-jkgiGD.js";import{_ as O}from"./plugin-vue_export-helper-x3n3nnut.js";const _=[{path:"/",title:"Home",pathLocale:"/",contents:[]},{path:"/Animation/MotionMatching.html",title:"Code vs Data Driven Displacement",pathLocale:"/",contents:[{header:"Code vs Data Driven Displacement",slug:"code-vs-data-driven-displacement",content:"原文地址：https://theorangeduck.com/page/code-vs-data-driven-displacement"},{header:"Definitions",slug:"definitions",content:""},{header:"The Simulation Object",slug:"the-simulation-object",content:`把手柄输入转化为预测要移动的物体。 vec3 desired_velocity_update( const vec3 gamepadstick_left, const float camera_azimuth, const quat simulation_rotation, const float fwrd_speed, const float side_speed, const float back_speed)
...
}`},{header:"The Character Entity",slug:"the-character-entity",content:`这就是玩家可以看到角色。角色的移动数据是来自动画切片，这种方式叫作数据驱动。
动画切片：https://github.com/ubisoft/ubisoft-laforge-animation-dataset`},{header:"Simulation Bone",slug:"simulation-bone",content:"模拟骨骼是给模拟对象使用的，用来代表移动旋转。"},{header:"Character Controller",slug:"character-controller",content:""},{header:"Synchronization",slug:"synchronization",content:`有两种方式： 直接使用模拟物体同步到角色：会出现滑步
由数据驱动模拟物体：这会造成延迟感 那么想到的方式就是直接混合这两者。`},{header:"Adjustment",slug:"adjustment",content:""}]},{path:"/Animation/note.html",title:"Note",pathLocale:"/",contents:[{header:"Note",slug:"note",content:`动画压缩
...
Consider enforcing their use by making the corresponding constructors private.`},{header:"Don't Be Cute",slug:"don-t-be-cute",content:`Cute name
...
Today we have source code control systems, we don't need this type of logs.`},{header:"Noise Comments",slug:"noise-comments",content:`The comments in the follow examples doesn't provides new information.
/**
* Default constructor.
*/
...
You could break the code in small functions instead to use this type of comments.`},{header:"Attributions and Bylines",slug:"attributions-and-bylines",content:`Example:
/* Added by Rick */
...
If you don't need anymore, please delete it, you can back later with your VCS if you need it again.`},{header:"HTML Comments",slug:"html-comments",content:`HTML in source code comments is an abomination, as you can tell by reading the code below.
/**
...
一定是带有业务特色的，比如我会去造一个业务组件库，但是我绝不会去造一个antd。`}]},{path:"/Career/performance.html",title:"Performance",pathLocale:"/",contents:[{header:"Performance",slug:"performance",content:""},{header:"主要表现及能力",slug:"主要表现及能力",content:`态度/承诺/积极性
专业精神和优秀品质
团队合作及信任`},{header:"专业技能",slug:"专业技能",content:`程式设计
架构
解决问题`}]},{path:"/Graphic/",title:"Graphic",pathLocale:"/",contents:[{header:"Graphic",slug:"graphic",content:""},{header:"OpenSource",slug:"opensource",content:"https://github.com/Gforcex/OpenGraphic"}]},{path:"/Playing/android-linux.html",title:"安装Linux在安卓手机上",pathLocale:"/",contents:[{header:"安装Linux在安卓手机上",slug:"安装linux在安卓手机上",content:`直接使用软件安装：https://github.com/EXALAB/AnLinux-App
...
可以输入：ssh-keygen -A 手动载入`},{header:"proot-distro",slug:"proot-distro",content:`apt install proot proot-distro -y
安装Linux系统的软件，使用proot-distro list来查看可以安装的系统
在对应系统中安装openssh后，需要手动设置以下sshd的配置：
vim /etc/ssh/sshd_config
添加一个特殊的端口，如9022。然后设置Permit Root Login yes:`},{header:"prm",slug:"prm",content:`切换npm源的小软件：npm i prm-cli -g
# List all registries
prm ls
...
x86-64-minimal-JIT-compiler-Cpp https://github.com/sol-prog/x86-64-minimal-JIT-compiler-Cpp/tree/master`}]},{path:"/Playing/router.html",title:"路由器",pathLocale:"/",contents:[{header:"路由器",slug:"路由器",content:`梅林安装教程：
...
这里就是把与CMakeLists.txt同目录下的TutorialConfig.h.in文件拷贝到build文件夹里。当然复制目录和生成目录都可以自定义设置。
...
第一个参数为库名字，后面加上需要编入库的文件，当然可以使用set、file等命令来把多个文件集合为一个变量。
把库工程连接到执行工程中，首先需要链接库的CMakeLists文件，使用add_subdirectory()命令。传入库文件夹的相对于执行工程的CMakeLists文件的目录即可。如下目录：
│ CMakeLists.txt
│ tutorial.cxx
│ TutorialConfig.h.in
│
└───MathFunctions CMakeLists.txt MathFunctions.h mysqrt.cxx
使用add_subdirectory(MathFunctions)即可，然后使用target_link_libraries()命令把库链接到执行文件中。`},{header:"Making Our Library Optional",slug:"making-our-library-optional",content:`在项目中会使用一些配置来控制工程，使用option(<variable> "<help_text>" [value])命令来控制。
...
│ TutorialConfig.h
│
└───lib MathFunctions.lib
文件目录可以自定义，主要使用install()命令进行配置。主要分为三种： bin：可执行文件 install(TARGETS Tutorial DESTINATION bin)
...
这里生成的就是install配置安装的包体。`},{header:"Selecting Static or Shared Libraries",slug:"selecting-static-or-shared-libraries",content:`使用BUILD_SHARED_LIBS字段来控制所有库共享属性，使用如下：
option(BUILD_SHARED_LIBS "Global flag to cause add_library() to create shared libraries if on" ON)
开关设置为ON/OFF。`},{header:"Adding Export Configuration",slug:"adding-export-configuration",content:"设置自定义命令"},{header:"Packaging Debug and Release",slug:"packaging-debug-and-release",content:""},{header:"常用命令",slug:"常用命令",content:`直接使用cmake命令在拥有CMakeLists.txt的目录下，会自动识别当前的运行的平台，来设置编译器和平台等。
需要特殊设置可以使用以下命令：
...
CMAKE_CURRENT_SOURCE_DIR：当前CMakeLists文件目录`},{header:"常用语句设置",slug:"常用语句设置",content:""},{header:"target_include_directories",slug:"target-include-directories",content:`target_include_directories(Tutorial PUBLIC \${PROJECT_BINARY_DIR})
文档
...
如果USE_BAR为ON，那么USE_FOO为ON，否则为OFF。简单来说就是依赖项为真，那么选项的值就为前面，反之为后面。
...
\${PROJECT_NAME}_src
\${PROJECT_NAME}_libsrc)`},{header:"报错",slug:"报错",content:`Failed to run MSBuild command: C:/Program Files (x86)/Microsoft Visual Studio/2019/Professional/MSBuild/Current/Bin/MSBuild.exe to get the value of VCTargetsPath: Access violation
解决：https://stackoverflow.com/questions/61842794/cmake-cant-get-the-value-of-vctargetspath-when-generating-for-uwp
...
https://modern-cmake-cn.github.io/Modern-CMake-zh_CN/`}]},{path:"/Tools/Git.html",title:"Git",pathLocale:"/",contents:[{header:"Git",slug:"git",content:""},{header:"SSH",slug:"ssh",content:`https://gitee.com/help/articles/4229#article-header0
https://www.cnblogs.com/cangqinglang/p/11727867.html
https://docs.github.com/cn/free-pro-team@latest/github/authenticating-to-github/connecting-to-github-with-ssh`},{header:"使用git把某一次commit修改过的文件打包",slug:"使用git把某一次commit修改过的文件打包",content:`把新增加的文件使用git add添加进改动
使用git commit提交改动
...
如果更新补下来子集，可以直接进入子集文件夹 ，然后clone下来即可。
把修改的文件打包`},{header:"两个远程仓库地址",slug:"两个远程仓库地址",content:`添加远程
git remote add 仓库A url
git remote add 仓库B url
git remote -v
合并其他源到本地分支
git merge remote/branch`},{header:"SSH 连接不上",slug:"ssh-连接不上",content:`既然和GitHub建立ssh连接的时候提示connection refused，那我们就详细看看建立ssh连接的过程中发生了什么，可以使用ssh -v命令，-v表示verbose，会打出详细日志。
...
The filename, directory name, or volume label syntax is incorrect. 2 build system warning(s): - Checking MSBuild version - Visual Studio does not include an English language pack
解决方案： On your machine, in the Window Search, type in Region
...
You can try a build to see if this takes away the messages, but a machine restart may be needed after making any changes. https://stackoverflow.com/questions/77229261/incredibuild-add-in-fails-to-build-c-solution-in-visual-studio`}]},{path:"/Web/Netcore.html",title:"Netcore",pathLocale:"/",contents:[{header:"Netcore",slug:"netcore",content:`https://www.gamereplays.org/overwatch/portals.php?show=page&name=overwatch-a-guide-to-understanding-netcode
...
在打包文件夹里执行http-server即可。`}]},{path:"/Animation/CharacterAnimation/",title:"CharacterAnimation",pathLocale:"/",contents:[{header:"CharacterAnimation",slug:"characteranimation",content:""}]},{path:"/Animation/CharacterAnimation/SkeletalAnimation.html",title:"Skeletal Animation",pathLocale:"/",contents:[{header:"Skeletal Animation",slug:"skeletal-animation",content:""},{header:"Keyframe animation basics",slug:"keyframe-animation-basics",content:`帧动画由一段时间中几个特定时间点和物体姿势组成。如下图：
Alt text
...
https://github.com/KhronosGroup/glTF-Tutorials/blob/master/gltfTutorial/gltfTutorial_006_SimpleAnimation.md`}]},{path:"/Animation/games103/RigidBody.html",title:"刚体",pathLocale:"/",contents:[{header:"刚体",slug:"刚体",content:""},{header:"什么是刚体？",slug:"什么是刚体",content:"一个物体表面很结实，它没有形变"},{header:"刚体模拟",slug:"刚体模拟",content:"更新不同时间点物体的状态"},{header:"刚体运动",slug:"刚体运动",content:`只允许刚体平移和旋转
...
F''（x）< 0 ,最大值
如果二阶导数永远大于0，那么函数没有最大值，存在唯一的一个最小值。`},{header:"Simulation by Newton`s Method",slug:"simulation-by-newton-s-method",content:""},{header:"Implicit Simulation",slug:"implicit-simulation",content:"我们就对每一个顶点模拟，计算它的速度与位置。"},{header:"Step 1 Initial",slug:"step-1-initial",content:`初始速度为重力速度：vi =(vi+dt∗g)∗damping
然后计算初步预测每个点的位置：xi~=xi+dt∗vi
同步初步预测位置到每个点：xi=xi~`},{header:"Step 2 Grandient Calculation",slug:"step-2-grandient-calculation",content:`我们使用隐式积分计算速度与位置是：
...
{gi=gi+k(1−Le||Xi−Xj||)(Xi−Xj)gj=gj−k(1−Le||Xi−Xj||)(Xi−Xj)`},{header:"Step 3 Finishing",slug:"step-3-finishing",content:`我们这里使用简单的方式来代替计算Hessian矩阵，更新位置使用梯度的公式：
Xi=Xi−(1Δt2mi+4k)−1gi同时再更新速度：
V=V+1Δt(X−X~)`},{header:"Bending and Locking Issues",slug:"bending-and-locking-issues",content:""},{header:"A CO-Rotational Method",slug:"a-co-rotational-method",content:""},{header:"The Locking Issue",slug:"the-locking-issue",content:`拉伸和弯曲时独立的问题
...
https://www.bilibili.com/video/BV12Q4y1S73g`}]},{path:"/Basic/algorithm/",title:"算法",pathLocale:"/",contents:[{header:"算法",slug:"算法",content:""}]},{path:"/Basic/design/Bottom-UpAndTop-Down.html",title:"自底向上与自顶向下的区别",pathLocale:"/",contents:[{header:"自底向上与自顶向下的区别",slug:"自底向上与自顶向下的区别",content:""},{header:"自顶向下模型",slug:"自顶向下模型",content:`在这种设计模型中，这个系统时定制的，我们并不知道系统每个部分的细节是什么。在每个部分中还有更多的细节。如果我们去深入了解一个这样设计的程序，需要一定是时间。这个我们做项目计划一样，我们已经有一个目标。我需要做的就是： 把大的目标分散成小目标
...
定点数在计算是会更快一些
在计算除法的时候，一般都会比较慢，我们可以使用左移来替换。`}]},{path:"/Basic/math/comparing-floats.html",title:"Comparing Floats",pathLocale:"/",contents:[{header:"Comparing Floats",slug:"comparing-floats",content:"https://bitbashing.io/comparing-floats.html"}]},{path:"/Basic/math/matrix.html",title:"Matrix",pathLocale:"/",contents:[{header:"Matrix",slug:"matrix",content:`https://betterexplained.com/articles/matrix-multiplication/
...
Availability=Uptime(Uptime+Downtime)If availability is 99.00% available, it is said to have "2 nines" of availability, and if it is 99.9%, it is called "3 nines", and so on. Availability (Percent)
Downtime (Year)
Downtime (Month)
...
simple-hashing
Hash(key1)→H1modN=Node0Hash(key2)→H2modN=Node1Hash(key3)→H3modN=Node2...Hash(keyn)→HnmodN=Noden−1Where,
...
Hash(key1)=P1Hash(key2)=P2Hash(key3)=P3...Hash(keyn)=Pm−1Where,
...
Hash1(key1)=P1Hash2(key2)=P2Hash3(key3)=P3...Hashk(keyn)=Pm−1Where,
key: Request/Node ID or IP.
...
For reads per month:
100×100 million=10 billion/monthSimilarly for writes:
1×100 million=100 million/monthWhat would be Requests Per Second (RPS) for our system?
100 million requests per month translate into 40 requests per second.
100 million(30 days×24 hrs×3600 seconds)=∼40 URLs/secondAnd with a 100:1 read/write ratio, the number of redirections will be:
100×40 URLs/second=4000 requests/second`},{header:"Bandwidth",slug:"bandwidth",content:`Since we expect about 40 URLs every second, and if we assume each request is of size 500 bytes then the total incoming data for write requests would be:
40×500 bytes=20 KB/secondSimilarly, for the read requests, since we expect about 4K redirections, the total outgoing data would be:
4000 URLs/second×500 bytes=∼2 MB/second`},{header:"Storage",slug:"storage-2",content:`For storage, we will assume we store each link or record in our database for 10 years. Since we expect around 100M new requests every month, the total number of records we will need to store would be:
100 million×10 years×12 months=12 billionLike earlier, if we assume each stored record will be approximately 500 bytes. We will need around 6TB of storage:
12 billion×500 bytes=6 TB`},{header:"Cache",slug:"cache",content:`For caching, we will follow the classic Pareto principle also known as the 80/20 rule. This means that 80% of the requests are for 20% of the data, so we can cache around 20% of our requests.
Since we get around 4K read or redirection requests each second, this translates into 350M requests per day.
4000 URLs/second×24 hours×3600 seconds=∼350 million requests/dayHence, we will need around 35GB of memory per day.
20 percent×350 million×500 bytes=35 GB/day`},{header:"High-level estimate",slug:"high-level-estimate",content:`Here is our high-level estimate: Type
...
625=∼916 million URLs626=∼56.8 billion URLs627=∼3.5 trillion URLsThis is the simplest solution here, but it does not guarantee non-duplicate or collision-resistant keys.
MD5 Approach
...
MD5(original_url)→base62encode→hashHowever, this creates a new issue for us, which is duplication and collision. We can try to re-compute the hash until we find a unique one but that will increase the overhead of our systems. It's better to look for more scalable approaches.
Counter Approach
...
Counter(0−3.5 trillion)→base62encode→hashThe problem with this approach is that it can quickly become a single point for failure. And if we run multiple instances of the counter we can have collision as it's essentially a distributed system.
...
6 characters×56.8 billion=∼390 GBWhile 390 GB seems like a lot for this simple use case, it is important to remember this is for the entirety of our service lifetime and the size of the keys database would not increase like our main database.`},{header:"Caching",slug:"caching-1",content:`Now, let's talk about caching. As per our estimations, we will require around ~35 GB of memory per day to cache 20% of the incoming requests to our services. For this use case, we can use Redis or Memcached servers alongside our API server.
...
50 million×20 messages=2 billion/dayMessages can also contain media such as images, videos, or other files. We can assume that 5 percent of messages are media files shared by the users, which gives us additional 200 million files we would need to store.
5 percent×2 billion=200 million/dayWhat would be Requests Per Second (RPS) for our system?
2 billion requests per day translate into 24K requests per second.
2 billion(24 hrs×3600 seconds)=∼24K requests/second`},{header:"Storage",slug:"storage-3",content:`If we assume each message on average is 100 bytes, we will require about 200 GB of database storage every day.
2 billion×100 bytes=∼200 GB/dayAs per our requirements, we also know that around 5 percent of our daily messages (100 million) are media files. If we assume each file is 50 KB on average, we will require 10 TB of storage every day.
100 million×100 KB=10 TB/dayAnd for 10 years, we will require about 38 PB of storage.
(10 TB+0.2 TB)×10 years×365 days=∼38 PB`},{header:"Bandwidth",slug:"bandwidth-1",content:`As our system is handling 10.2 TB of ingress every day, we will require a minimum bandwidth of around 120 MB per second.
10.2 TB(24 hrs×3600 seconds)=∼120 MB/second`},{header:"High-level estimate",slug:"high-level-estimate-1",content:`Here is our high-level estimate: Type
...
200 million×5 tweets=1 billion/dayTweets can also contain media such as images, or videos. We can assume that 10 percent of tweets are media files shared by the users, which gives us additional 100 million files we would need to store.
10 percent×1 billion=100 million/dayWhat would be Requests Per Second (RPS) for our system?
1 billion requests per day translate into 12K requests per second.
1 billion(24 hrs×3600 seconds)=∼12K requests/second`},{header:"Storage",slug:"storage-4",content:`If we assume each message on average is 100 bytes, we will require about 100 GB of database storage every day.
1 billion×100 bytes=∼100 GB/dayWe also know that around 10 percent of our daily messages (100 million) are media files per our requirements. If we assume each file is 50 KB on average, we will require 5 TB of storage every day.
100 million×100 KB=5 TB/dayAnd for 10 years, we will require about 19 PB of storage.
(5 TB+0.1 TB)×365 days×10 years=∼19 PB`},{header:"Bandwidth",slug:"bandwidth-2",content:`As our system is handling 5.1 TB of ingress every day, we will require a minimum bandwidth of around 60 MB per second.
5.1 TB(24 hrs×3600 seconds)=∼60 MB/second`},{header:"High-level estimate",slug:"high-level-estimate-2",content:`Here is our high-level estimate: Type
Estimate Daily active users (DAU)
...
Rank=Affinity×Weight×DecayWhere,
...
200 million×5 videos=1 billion/dayAssuming a 200:1 read/write ratio, about 50 million videos will be uploaded every day.
1200×1 billion=50 million/dayWhat would be Requests Per Second (RPS) for our system?
1 billion requests per day translate into 12K requests per second.
1 billion(24 hrs×3600 seconds)=∼12K requests/second`},{header:"Storage",slug:"storage-5",content:`If we assume each video is 100 MB on average, we will require about 5 PB of storage every day.
50 million×100 MB=5 PB/dayAnd for 10 years, we will require an astounding 18,250 PB of storage.
5 PB×365 days×10 years=∼18,250 PB`},{header:"Bandwidth",slug:"bandwidth-3",content:`As our system is handling 5 PB of ingress every day, we will require a minimum bandwidth of around 58 GB per second.
5 PB(24 hrs×3600 seconds)=∼58 GB/second`},{header:"High-level estimate",slug:"high-level-estimate-3",content:`Here is our high-level estimate: Type
Estimate Daily active users (DAU)
...
100 million×10 actions=1 billion/dayWhat would be Requests Per Second (RPS) for our system?
1 billion requests per day translate into 12K requests per second.
1 billion(24 hrs×3600 seconds)=∼12K requests/second`},{header:"Storage",slug:"storage-6",content:`If we assume each message on average is 400 bytes, we will require about 400 GB of database storage every day.
1 billion×400 bytes=∼400 GB/dayAnd for 10 years, we will require about 1.4 PB of storage.
400 GB×10 years×365 days=∼1.4 PB`},{header:"Bandwidth",slug:"bandwidth-4",content:`As our system is handling 400 GB of ingress every day, we will require a minimum bandwidth of around 4 MB per second.
400 GB(24 hrs×3600 seconds)=∼5 MB/second`},{header:"High-level estimate",slug:"high-level-estimate-4",content:`Here is our high-level estimate: Type
Estimate Daily active users (DAU)
100 million Requests per second (RPS)
12K/s Storage (per day)
...
开始运行时，先将7，0，,三个页面装入内存。当进程要访问页面2时，产生缺页中断，会将页面7换出，因为页面7再次被访问的时间最长。`},{header:"2.最近最久未使用 LRU, Least Recently Used",slug:"_2-最近最久未使用-lru-least-recently-used",content:`虽然无法知道将来要使用的页面情况，但是可以知道过去使用页面的情况。LRU 将最近最久未使用的页面换出。
...
https://solarianprogrammer.com/2012/10/17/cpp-11-async-tutorial/`},{header:"Async loading data example",slug:"async-loading-data-example-1",content:"https://www.youtube.com/watch?v=2eJm6tA4y2U"},{header:"Multi-thread Renderingthread",slug:"multi-thread-renderingthread",content:""}]},{path:"/Career/interview/interview.html",title:"面试",pathLocale:"/",contents:[{header:"面试",slug:"面试",content:""},{header:"你负责的业务是什么？(学会发现问题)",slug:"你负责的业务是什么-学会发现问题",content:"业务最核心的要素是业务本身的价值"},{header:"可以开始写代码了？(学会思考的方式)",slug:"可以开始写代码了-学会思考的方式",content:""},{header:"图，是思想的结晶",slug:"图-是思想的结晶",content:`第一步，先想明白这张图要表达什么？
...
This work is licensed under a Creative Commons Attribution-ShareAlike 4.0 International License.`}]},{path:"/GameEngine/Godot/Debugging.html",title:"Debugging",pathLocale:"/",contents:[{header:"Debugging",slug:"debugging",content:""},{header:"Viusal Studio",slug:"viusal-studio",content:"需要设置一下调试，如下操作： 设置一下，引擎路径即可。"}]},{path:"/GameEngine/Unreal/References.html",title:"References",pathLocale:"/",contents:[{header:"References",slug:"references",content:`代码规范
资源命名规范 类与结构体 http://jollymonsterstudio.com/2018/12/07/ue4-c-fundamentals-structs/
https://iq.opengenus.org/structure-vs-class-in-cpp/ Deattach后再Attach无法调试的问题。可以使用UE编辑器编译一次，需要关闭Live Coding。`}]},{path:"/Gameplay/AI/GameAIModel.html",title:"游戏AI模型",pathLocale:"/",contents:[{header:"游戏AI模型",slug:"游戏ai模型",content:`一般游戏中的AI模块我们都可以它分为三个小块： 移动
行为决策
...
EntityProcess ：含有所有逻辑处理器且驱动它们。`},{header:"源码",slug:"源码",content:"GitHub"}]},{path:"/Gameplay/design/",title:"参考",pathLocale:"/",contents:[{header:"参考",slug:"参考",content:`https://www.romsgames.net/gameboy-advance-rom-chocobo-land---game-de-dice/
...
我们看到代码中有一个变量 IsLittleEndian，它代表这台计算机是大端编码还是小端编码，不同的计算机编码方式会有不同。所以问题就是，不同编码方式下，计算方法不同，那对与不同的计算机，读取出来的数据长度肯定是不同的，这个是需要我们处理。`},{header:"什么是大端小端？",slug:"什么是大端小端",content:`我们知道在计算机中所有数据都是用二进制表示的，举个例子，如果用16位二进制表示数字 258 ，它的二进制是 0000000100000010 ，转换成16进制是 0x0102 。
假如使用大端模式存入内存，内存数据如图： 还原这个数字的步骤是： 拿到第1个字节的数据 00000001，乘以进制位256（2的8次方），得到256，即第1个字节（低地址）代表了十进制数字256；
拿到第2个字节的数据 00000010 ，它代表十进制数字2，乘以进制位1，得到2；
将前两步得到的数字相加，即256+2，得到258，还原出数字。 假如使用小端模式存入内存，内存数据如图： 还原这个数字的步骤是： 拿到第1个字节的数据 00000010 ，它代表十进制数字2，乘以进制位1，得到2；
拿到第2个字节的数据 00000001，乘以进制位256（2的8次方），得到256，即第1个字节（低地址）代表了十进制数字256；
...
一些简单的unity shader https://github.com/BanMing/BanMingShaderLab`}]},{path:"/Platform/windows/win32.html",title:"Note",pathLocale:"/",contents:[{header:"Note",slug:"note",content:`WNDCLASSEX, RegisterClassEx, CreateWindowEx 与 WNDCLASSEXA, RegisterClassExA, CreateWindowExA
前者是显示的创建一个窗口，后者是隐藏创建一个窗口。`}]},{path:"/Tools/Windows/AccessIsDenied.html",title:"How to fix - Error 5: Access is Denied in Windows 10",pathLocale:"/",contents:[{header:"How to fix - Error 5: Access is Denied in Windows 10",slug:"how-to-fix-error-5-access-is-denied-in-windows-10",content:`Error indicates that you wont have required privileges to install the application to that particular system drive. Right-click on the application(.exe) file and select Run as administrator installing the same. Do follow the below mentioned steps and heck if it helps.
Method 1:
...
./emsdk_env.bat
运行后所有需要的文件都在emsdk/upstream/文件夹中，需要把emsdk/upstream/bin设置到环境变量。
...
最后需要安装MinGW。`},{header:"Mac",slug:"mac",content:`命令安装：
...
}`}]},{path:"/Basic/language/Cplusplus/SmartPtr.html",title:"智能指针",pathLocale:"/",contents:[{header:"智能指针",slug:"智能指针",content:""},{header:"unique_ptr",slug:"unique-ptr",content:"脱离作用域，该指针就会被释放。不能复制这个指针。"}]},{path:"/Basic/language/Cplusplus/vector.html",title:"Vector",pathLocale:"/",contents:[{header:"Vector",slug:"vector",content:""},{header:"emplace_back",slug:"emplace-back",content:`使用 emplace_back 添加数列中的元数可以减少复制构造函数的调用。但是前提是传入的是该数组类型构造函数的参数。例如：
class Vertex
{
...
（4）lua 比如：for 空洞的 table。寄存器语言特性, lua vm 一些基本的概念`}]},{path:"/GameEngine/Unity/manual/",title:"Manual",pathLocale:"/",contents:[{header:"Manual",slug:"manual",content:""}]},{path:"/GameEngine/Unity/somecode/Framework%20Test.html",title:"Framework Test",pathLocale:"/",contents:[{header:"Framework Test",slug:"framework-test",content:""},{header:"代码放哪里？",slug:"代码放哪里",content:`在项目中代码是直接新建一个C#的solution，直接生成dll导入unity工程中，这样可以把代码和资源彻底分开，这样也非常适合分远程库。但是有一个致命的问题就是IL2CPP，这样就需要自己接入IL2CPP到C#工程里去。暂时还没有找到相应的教程指导我们生成IL2CPP文件。这里有个最差的想法就是在打包的时候，把代码复制到unity工程中去，这样就可以使用IL2CPP。
...
BungeeMan | Tutorials
Content Examples`}]},{path:"/GameEngine/Unreal/animation/PlayerMove.html",title:"Player Move",pathLocale:"/",contents:[{header:"Player Move",slug:"player-move",content:`指引
https://docs.unrealengine.com/4.27/zh-CN/InteractiveExperiences/HowTo/CharacterMovement/`}]},{path:"/GameEngine/Unreal/animation/ProceduralAniamtionForHumans.html",title:"Procedural Aniamtion for humans",pathLocale:"/",contents:[{header:"Procedural Aniamtion for humans",slug:"procedural-aniamtion-for-humans",content:"Udemy:https://www.udemy.com/course/procedural-animation/"}]},{path:"/GameEngine/Unreal/course/rpg.html",title:"Unreal Engine 5 Gas Top Down RPG",pathLocale:"/",contents:[{header:"Unreal Engine 5 Gas Top Down RPG",slug:"unreal-engine-5-gas-top-down-rpg",content:"https://www.udemy.com/course/unreal-engine-5-gas-top-down-rpg/learn/lecture/37324564?start=525#overview"},{header:"Notes",slug:"notes",content:`关闭 Live Code， 直接使用VS编译即可。
关闭 Automaticlly Complie Newly Added C++ Classes
VS设置插件,在安装后需要手动设置一下uproject文件，文档。需要在商城里下载或者直接在插件中心打开。 ,{ "Name": "VisualStudioTools", "Enabled": true } 如果发现编译出来的效果不对，删除动态链接库，重新加载即可。`},{header:"Enhanced Input",slug:"enhanced-input",content:"[]"},{header:"Gameplay Ability System",slug:"gameplay-ability-system",content:""},{header:"UI",slug:"ui",content:"HUD -> Controller - > View"}]},{path:"/GameEngine/Unreal/manual/BP2C__.html",title:"蓝图转C++",pathLocale:"/",contents:[{header:"蓝图转C++",slug:"蓝图转c",content:`可以设置蓝图转化成C++，来解决一些性能上的问题。标记需要生成C++的蓝图就不会打到pak包中。
...
估计还可以不用修改引擎代码来实现，比如说修改批处理的执行、监听某些事件来处理。应该是重写Engine\\Engine\\Source\\Programs\\AutomationTool\\AutomationUtils\\Platform.cs这个类来实现。`}]},{path:"/GameEngine/Unreal/manual/Commandline.html",title:"Command Line",pathLocale:"/",contents:[{header:"Command Line",slug:"command-line",content:""},{header:"读取启动命令行命令",slug:"读取启动命令行命令",content:'```cpp\nFString ParameterValue;\nif (FParse::Value(FCommandLine::Get(), TEXT("-LocalClientIndex="), ParameterValue))\n{ LocalUserIndex = FCString::Atoi((*ParameterValue)); UE_LOG(LogBalor, Log, TEXT("Command Line LocalUserIndex@@@@@@: %s "), *ParameterValue);\n}\n```'},{header:"控制台命令行调用",slug:"控制台命令行调用",content:`直接在方法属性中添加Exec即可，在控制台直接使用函数名调用。
UFUNCTION( Exec )
void Test(const FString TestStr)
多人模式下，服务器可以直接使用函数名调用，客户端需要在函数名前加上ServerExec来调用。`}]},{path:"/GameEngine/Unreal/manual/Component.html",title:"Component",pathLocale:"/",contents:[{header:"Component",slug:"component",content:""},{header:"GetComponentByClass and FindCompoentByClass",slug:"getcomponentbyclass-and-findcompoentbyclass",content:`GetComponentByClass是给蓝图调用的，底层就是用FindCompoentByClass
...
C++ 函数在UFUNCTION声明中添加标记符CallinEditor UFUNCTION(BlueprintCallable, CallInEditor, Category = "Default") void TestFunction();
Alt text`},{header:"编辑器工具控件 / 编辑器工具蓝图",slug:"编辑器工具控件-编辑器工具蓝图",content:"蓝图函数库（BlueprintFunctionLibrary）"},{header:"Python",slug:"python",content:""},{header:"打印字符串",slug:"打印字符串",content:`固定屏幕打印字符串的位置： 使用Key属性替换HUD中已有的Log
C++下：GEngine->AddOnScreenDebugMessage
仅非Shipping下有效果 Alt text`},{header:"日志",slug:"日志",content:`保存在Saved/Logs
...
- 打包时隐藏符号：-nodebuginfo`},{header:"调试工具",slug:"调试工具",content:""},{header:"Unreal Insights",slug:"unreal-insights",content:`启动方式： -trace=default,task,memory,cook -tracehost=127.0.0.1
...
Instruments Alt text`},{header:"GPU调试 - DumpGPU",slug:"gpu调试-dumpgpu",content:"Alt text"}]},{path:"/GameEngine/Unreal/manual/EnhancedInput.html",title:"Enhanced Input",pathLocale:"/",contents:[{header:"Enhanced Input",slug:"enhanced-input",content:""},{header:"Enable",slug:"enable",content:`在Build.cs中的PublicDependencyModuleNames添加EnhancedInput。
...
https://www.bilibili.com/video/BV14r4y1r7nz/`}]},{path:"/GameEngine/Unreal/manual/GASDocumentation.html",title:"GASDocumentation",pathLocale:"/",contents:[{header:"GASDocumentation",slug:"gasdocumentation",content:`原地址
...
I recommend using the GameplayTag version over the FName version. This can prevent spelling errors in Blueprint.`},{header:"4.5.10 Gameplay Effect Context",slug:"_4-5-10-gameplay-effect-context",content:`The GameplayEffectContext structure holds information about a GameplayEffectSpec's instigator and TargetData. This is also a good structure to subclass to pass arbitrary data around between places like ModifierMagnitudeCalculations / GameplayEffectExecutionCalculations, AttributeSets, and GameplayCues.
...
InfoGold.Attribute = UGDAttributeSetBase::GetGoldAttribute(); Source->ApplyGameplayEffectToSelf(GEBounty, 1.0f, Source->MakeEffectContext());
...
AbilitySystemComponent->BindAbilityActivationToInputComponent(PlayerInputComponent, FGameplayAbilityInputBinds(FString("ConfirmTarget"), FString("CancelTarget"), AbilityEnumAssetPath, static_cast<int32>(EGDAbilityInputID::Confirm), static_cast<int32>(EGDAbilityInputID::Cancel)));
...
Activation Failed Tags Displayed in showdebug AbilitySystem`},{header:"4.6.5 Canceling Abilities",slug:"_4-6-5-canceling-abilities",content:`To cancel a GameplayAbility from within, you call CancelAbility(). This will call EndAbility() and set its WasCancelled parameter to true.
...
Once you get the FGameplayAbilitySpec that you are looking for, you can call IsActive() on it.`},{header:"4.6.7 Instancing Policy",slug:"_4-6-7-instancing-policy",content:`A GameplayAbility's Instancing Policy determines if and how the GameplayAbility is instanced when activated. Instancing Policy
Description
...
log list
Notable GAS related logging categories: Logging Category
...
See Engine\\Source\\Runtime\\CoreUObject\\Public\\UObject\\TopLevelAssetPath.h for more info.`},{header:"10. Common GAS Acronyms",slug:"_10-common-gas-acronyms",content:`Name
...
#include "AbilitySystemComponent.h" #define ATTRIBUTE_ACCESSORS(ClassName, PropertyName) \\ GAMEPLAYATTRIBUTE_PROPERTY_GETTER(ClassName, PropertyName) \\ GAMEPLAYATTRIBUTE_VALUE_GETTER(PropertyName) \\ GAMEPLAYATTRIBUTE_VALUE_SETTER(PropertyName) \\ GAMEPLAYATTRIBUTE_VALUE_INITTER(PropertyName)
头文件实列： UPROPERTY(BlueprintReadOnly, ReplicatedUsing = OnRep_Health, Category = "Vital Attributes") FGameplayAttributeData Health; ATTRIBUTE_ACCESSORS(ThisClass, Health); UFUNCTION() void OnRep_Health(const FGameplayAttributeData& OldHealth);
类实列：
...
}`},{header:"常用函数",slug:"常用函数",content:`PreAttributeChange：数据在修改前，调用。
PostAttributeChange：数据在修改后，调用。`},{header:"Gameplay Effect",slug:"gameplay-effect",content:`Alt text
执行规则： Instant 一次
Has Duration 一段时间
Infinite 无限次`},{header:"Periodic Gameplay Effects",slug:"periodic-gameplay-effects",content:`Alt text
在一段时间内定期修改基础数值`},{header:"Stacking",slug:"stacking",content:""},{header:"Aggregate by Soucre",slug:"aggregate-by-soucre",content:"Alt text"},{header:"Aggregate by Soucre",slug:"aggregate-by-soucre-1",content:"Alt text"},{header:"Gameplay Ability",slug:"gameplay-ability",content:""},{header:"Ability Task",slug:"ability-task",content:""},{header:"Gameplay Cue",slug:"gameplay-cue",content:""},{header:"Gameplay Tag",slug:"gameplay-tag",content:`Alt text
可以多出使用Tag： Inputs
Abilities
Attributes
Damage Types
...
使用DataTable创建GameplayTagTableRow数据表，需要配置到项目设置中的Gameplay Tag Table List GameplayEffect中的Tag配置：
Alt text Category
Description Gameplay Effect Asset Tags
Tags that the GameplayEffect has. They do not do any function on their own and serve only the purpose of describing the GameplayEffect. Granted Tags
Tags that live on the GameplayEffect but are also given to the ASC that the GameplayEffect is applied to. They are removed from the ASC when the GameplayEffect is removed. This only works for Duration and Infinite GameplayEffects. Ongoing Tag Requirements
...
将标签授予Gameplay效果的目标（有时指所有者）。 UTargetTagRequirementsGameplayEffectComponent
...
CORE_API DECLARE_LOG_CATEGORY_EXTERN(YourChannel, Log, All);
声明自定义频道与类型
...
UE_LOG(LogTemp, Warning, TEXT("Text, %d %f %s"), intVar, floatVar, *fstringVar );`}]},{path:"/GameEngine/Unreal/manual/NetWorkIDs.html",title:"NetWorkIDs",pathLocale:"/",contents:[{header:"NetWorkIDs",slug:"networkids",content:"FUniqueNetID vs FNetworkGUID vs PlayerId"},{header:"GetUniqueID()",slug:"getuniqueid",content:"这个是UObject的唯一ID，只有object是激活的。但是它并不是一样数值存在于服务器和客户端。"},{header:"FNetworkGUID",slug:"fnetworkguid",content:`这个是服务器和客户端统一的一个UObject的唯一ID。它是存在 UNetDriver 里的FNetGUICache中的。我们可以这样获取：
FNetworkGUID networkID = GetWorld()->GetNetDriver()->GuidCache->GetNetGUID(GetOwner());;`},{header:"APlayerState.PlayerId",slug:"aplayerstate-playerid",content:""},{header:"APlayerState.UniqueID",slug:"aplayerstate-uniqueid",content:"Reference From https://udn.unrealengine.com/questions/232509/about-testing-multiplayer.html"}]},{path:"/GameEngine/Unreal/manual/Note.html",title:"Note",pathLocale:"/",contents:[{header:"Note",slug:"note",content:`如果在build时出现cannot open file ... dll 需要把引擎的文件夹的read only去掉 Hitting D8049 errors when building editor
https://forums.unrealengine.com/t/hitting-d8049-errors-when-building-editor-from-ue4-sln/449409/4 关于UCLASS和UPROPERTY中的设置可以在UnrealEngine\\Engine\\Source\\Runtime\\CoreUObject\\Public\\UObject\\ObjectMacros.h文件中查看。或者直接引用#include "UObject/ObjectMacros.h"，直接使用UC::或者UP::会有提示。`}]},{path:"/GameEngine/Unreal/manual/UnrealEnginePipeline.html",title:"Unreal Engine Pipeline",pathLocale:"/",contents:[{header:"Unreal Engine Pipeline",slug:"unreal-engine-pipeline",content:""},{header:"UE4",slug:"ue4",content:""},{header:"UE5",slug:"ue5",content:""},{header:"Animation",slug:"animation",content:""},{header:"Blueprint",slug:"blueprint",content:""},{header:"Character",slug:"character",content:""},{header:"Materials",slug:"materials",content:""},{header:"Programming",slug:"programming",content:""},{header:"Rendering",slug:"rendering",content:""},{header:"World Building",slug:"world-building",content:""},{header:"Refence",slug:"refence",content:`https://dev.epicgames.com/community/learning/courses/7L/engine-structure-beginplay/98E/overview-engine-structure-beginplay
...
@echo off set Paks_Exe=..\\Engine\\Binaries\\Win64\\UnrealPak-Win64-Test.exe
...
加入服务器可以使用：
UGameplayStatics::OpenLevel(this, IPAddress);
或者使用： APlayerController* playerController = GetGameInstance()->GetFirstLocalPlayerController(); if (playerController) { playerController->ClientTravel(IPAddress, ETravelType::TRAVEL_Absolute); }`},{header:"Online Subsystem",slug:"online-subsystem",content:`在Engine.ini中设置使用哪个平台的服务器： 使用一下代码来获取网络系统：
...
ROLE_None 服务器上显示的角色类型如下图，其中最靠近屏幕的Pawn是本地控制角色： 客户端上显示的角色类型如下图，其中最靠近屏幕的Pawn是本地控制角色：`},{header:"Replicated",slug:"replicated",content:`UPROPERTY(Replicated)
...
class Heuristic: # Stores the goal node that this heuristic is estimating for. goalNode: Node # Estimated cost to reach the stored goal from the given node. function estimate(fromNode: Node) -> float: return estimate(fromNode, goalNode) # Estimated cost to move between any two nodes. function estimate(fromNode: Node, toNode: Node) -> float
...
class NodeRecord: node: Node connection: Connection costSoFar: float estimatedTotalCost: float category: {CLOSED, OPEN, UNVISITED}
如果我们使用类型来区分节点，那么我们是可以不需要close这个列表的。`},{header:"The Open List Implementation",slug:"the-open-list-implementation",content:`我们不能直接去掉open列表，因为还是需要求得最小的值。但是我们可以把记录改造成链表式，让每个记录点都记录着它后面的一个记录点。这样我们就需要修改节点的数据结构，如下：
# The structure used to track the information we need for each node.
class NodeRecord: node: Node connection: Connection costSoFar: float estimatedTotalCost: float category: {CLOSED, OPEN, UNVISITED} nextRecordInList: NodeRecord
...
x=xmin,x=xmax,y=ymin,y=ymax.(x,y)∈[xmin,xmax]×[ymin,ymax].`},{header:"分层包围盒",slug:"分层包围盒",content:`我们在检测碰撞时，可以设置包围盒。提前测试包围盒是否被射线穿过，然后再判断包围盒中的每个物体是否被穿透。这个看起来是减少了区域检测，但是我们在包围盒内部还是需要蛮力搜索。所以我们还可以把包围盒内部再次切割出不同的包围盒。 我们简单的伪代码可以写成这样：
...
我们可以看到上面这种切分包围盒到更小的包围盒，然后始查询，这个查询其实是没有一定的顺序规则，先查询那个后查询那个。这里我们可以使用二叉树来管理，如下图： 把一个包围盒中的点分为灰色和黑色。我们可以简单的写出每个节点的数据结构：
class bvh-node subclass of surface virtual bool hit(ray e + td, real t0, real t1, hit-record rec) virtual box bounding-box() surface-pointer left surface-pointer right box bbox
实现一个碰撞的方法：
function bool bvh-node::hit(ray a + tb, real t0, real t1,hit-record rec) if (bbox.hitbox(a + tb, t0, t1)) then hit-record lrec, rrec left-hit = (left != NULL) and (left -> hit(a + tb, t0, t1, lrec)) right-hit = (right != NULL) and (right -> hit(a+tb, t0, t1, rrec)) if (left-hit and right-hit) then if (lrec.t < rrec.t) then rec = lrec else rec = rrec return true else if (left-hit) then rec = lrec return true else if (right-hit) then rec = rrec return true else return false else return false
这里就是递归检测分别检测左边右边时候有碰撞到。
我们可以使用一个轴来划分包围盒，这个轴可以是x=0,y=1,z=2这样的，同时我们可以划分两部分包含的物体的个数相等，这样的话我们可以减少二叉树中有很多单子叶的情况。
...
假设向量a要旋转φ角度到向量b，且a向量本身距x轴有角度α: xb = r cos(α + φ) = r cos α cos φ − r sin α sin φ yb = r sin(α + φ) = r sin α cos φ + r cos α sin φ.
...
可以把一个向量转化为三角形，然后利用勾股定理转化为长度。`},{header:"点乘",slug:"点乘",content:""},{header:"数学",slug:"数学",content:"a与b点乘公式： 计算公式： 从公式可以看出向量点乘结果为一个标量。 点乘满足规则： p · p = ||p||²"},{header:"几何意义",slug:"几何意义",content:`给定两个向量v和n，将能v分解成两个分量：V∥和V⊥。它们分别平行于和垂直于n，并满足v = V∥ + V⊥。一般称平行分量V∥为v在n上的投影。 这里我们可以V∥表示为： 根据三角函数我们可以求出||V∥|| = cosθ||v||，这样我们就可以求到 我们可以根据点乘结果的符号可以判断两个向量的方向： 结果大于0，同向
结果等于0，垂直
...
function rasterize_triangle(t) for Xmin to Xmax do for Ymin to Ymax do if is_in_trangle(t，x，y) then r，g，b = compute_barycentric(x，y，t) draw_color(r，g，b)
...
function convolve(sequence a， filter b， int i) s = 0 r = b.radius for j = i − r to i + r do s = s + a[j]b[i − j] return s
上面的例子只是对应了一维的变换，也就是波形中y坐标的改变。我们需要对图片做卷积，那就是二维的，这里我们就会用到卷积核的一个东西，就是滤波器为一个矩形，如下图： 我们求一个点 a[i,j] 与数组 b 的卷积，b 就是一个卷积核，把这个矩形的中心 b[0,0] 放在 a[i,j] 上，然后把卷积核覆盖在 a 中对应的点分别乘上对应数组 b 中的值，把所有乘积就相加即可获得该点的卷积。 公式如： 简单的伪代码如下：
function convolve2d(sequence2d a， filter2d b， int i， int j) s = 0 r = b.radius for i′ = i − r to i + r do for j′ = j − r to j + r do s = s + a[i′][j′]b[i − i′][j − j′] return s
...
在光栅化工程中，不断更新其深度： for (each triangle T) for (each sample(x,y,z) in T) if (z < zbuffer[x,y]) { framebuffer[x,y] = rgb; zbuffer[x,y] = z; }
...
简单的伪代码如下：
y = y0
for x = x0 to x1 do draw(x, y) if f(x + 1, y + 0.5) < 0 then y = y + 1
这里使y坐标加一条件就是用到了直线与点的关系的规则： 如果点在直线上，f(x,y) = 0
如果点在直线上方，f(x,y) > 0
...
右：平面能接受的能量与该平面的法线和入射光的角度有关。其关系是：平面接受的能量的百分比 = cos(n,l) = n·l`},{header:"光的强度",slug:"光的强度",content:"我们明白了光照射到平面的方向的关系了，光的强度我们可以用烤火来理解，我们离火堆很近温度就高一些，离火堆远一点，温度就小一点，那我们就可以以火堆为圆心，火堆到我们的距离为半径，模拟成下图："},{header:"兰伯特光照模型",slug:"兰伯特光照模型",content:`接合上面两个因素我们就可以得到一个公式： 这其中： Ld：着色点的漫反射分量
Kd：漫反射系数，可调节。一般使用该着色点的RGB颜色。
(I/r2)：光照强度
max(0,n⋅l):如果光照方向与表面发现反方向，是不会接收到光照的，反方向点乘为负数，所以限制为0。 Kd数值对漫反射数值的影响：`},{header:"高光反射",slug:"高光反射",content:"我们平时看一些金属或者光滑的表面，我们会发现一些亮白色的块，那个就是高光。我们也可以容易的发现，当我们的眼睛移动时，物体表面的亮白色块也会移动。所以观察方向会影响到高光，相对漫反射中的条件就会多一个视角方向。这个亮块就是我们光源的反射造成的，它的颜色通常是光源的颜色。"},{header:"Phong",slug:"phong",content:`我们要模拟高光现象，用到了Phong光照模型，我们先看高光反射示意图： 图中所示： l：光源
n：法线
r：光源放射
...
然后写出最终公式：
Ls=ks⋅(Ir2)⋅max(0,v⋅r)p=ks⋅(Ir2)⋅max(0,v⋅(−l+2(l⋅n)n))p`},{header:"Blinn",slug:"blinn",content:`上述就是Phong光照模型，Blinn提出了一个简单的修改方法来得到类似的效果。它的基本思想是，避免计算反射方向 r。为此，Blinn模型引入了一个新的矢量 h，他是通过 l 和 v 的取平均后再归一化得到，也就是光源方向与视角方向的中间。根据我们漫反射公式，我们就可以写出最终公式： 公式中： Ls：高光反射
Ks：高光反射系数
...
Ka：材质的环境系数
Ia：场景的环境光强度 值得注意这个公式也是近似的算法。`},{header:"Blinn Phong 展示",slug:"blinn-phong-展示",content:`当我们计算出漫反射、高光、环境光之后，把它们加起来，就能得到 Blinn Phong 着色模型的结果了： Blinn Phong的着色公式为：L=La+Ld+Ls
L=kaIa+kd(Ir2)max(0,n⋅l)+ks(Ir2)max(0,n⋅h)p`},{header:"逐像素还是逐顶点",slug:"逐像素还是逐顶点",content:"上面，我们给出了基本光照模型使用的数学公式，那么我们在哪里计算这些光照模型呢？通常来讲，我们有两个选择：在片元着色器中计算，也被称为逐像素光照；在顶点着色器中计算，也被称为逐顶点光照。"},{header:"平面着色",slug:"平面着色",content:"平面着色是指整个三角面中共用一个方向的法线，因此表现为整个三角面的颜色都是一样的。如下图："},{header:"Phong着色",slug:"phong着色",content:`在逐像素光照中，我们会以每个像素为基础，得到它的法线（可以是对顶点法线插值得到的，也可以是从法线纹理中采样得到的），然后进行光照模型的计算。这种在面片之间对顶点法线进行插值的技术被称为Phong着色，也被称为Phong插值或法线插值着色技术。这不同于我们之前讲到的Phong光照模型。
...
我们是一般是在做漫反射时，Kd漫反射系数包含了纹理中某个区域的颜色，我们也可以直接使用顶点的颜色作为Kd。`},{header:"贴图映射",slug:"贴图映射",content:`那么我们是怎么把着色点和贴图联系起来的呢？这里就需要使用到uv坐标，然后映射关联起来。
...
然后找到Unity的安装路径，设置为UnityInstallPath，我们可以从这几个位置去查询Unity程序的位置： 判断UnityInstallPath这个值是否已经被设置，如使用userprops文件
...
我们可以直接使用pdb2mdb命令生成mdb：
pdb2mdb.exe unity_dll_lib.dll
不过mono4.5版本的pdb2mdb.exe移动出来单独执行会报错：
...
他是有几个引用项的，最后验证只需要依赖一下这两个dll： Mono.Cecil.dll（C:\\Program Files\\Unity\\2020.3.7f1\\Editor\\Data\\MonoBleedingEdge\\lib\\mono\\gac\\Mono.Cecil\\0.10.0.0__0738eb9f132ed756\\Mono.Cecil.dll）
...
同样我们也可以早早的定义好没有子物体的游戏物体的Transform.hierarchyCapacity`},{header:"Considering caching transform changes",slug:"considering-caching-transform-changes",content:`有两种优化这里： 能直接修改local本地的数据就修改本地的数据，这样省去了矩阵的转换，世界坐标到本地坐标
...
但是触发这个回调必须要有Render相关的组件才行，如：Skinned Mesh Renderer,Mesh Renderer 等。`},{header:"Disabling objects by distance",slug:"disabling-objects-by-distance",content:"距离远的物体可以关闭"},{header:"Using distance-squared over distance",slug:"using-distance-squared-over-distance",content:`使用开方消耗极大，我们使用sqrMagnitude来代替，例如：
...
UnityEditor.BuildOptions： BuildOptions.Development | BuildOptions.AllowDebugging
查看调试端口，可以在缓存目录看Player.log文件`},{header:"参考",slug:"参考",content:`https://docs.unity3d.com/ScriptReference/BuildOptions.html
https://docs.unity3d.com/Manual/EditorCommandLineArguments.html
https://docs.unity3d.com/Manual/UnityCloudBuildDevelopmentBuilds.html`}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]},{path:"/Animation/",title:"Animation",pathLocale:"/",contents:[]},{path:"/Basic/",title:"Basic",pathLocale:"/",contents:[]},{path:"/Career/",title:"Career",pathLocale:"/",contents:[]},{path:"/Playing/",title:"Playing",pathLocale:"/",contents:[]},{path:"/Tools/",title:"Tools",pathLocale:"/",contents:[]},{path:"/Web/",title:"Web",pathLocale:"/",contents:[]},{path:"/Basic/math/",title:"Math",pathLocale:"/",contents:[]},{path:"/Career/interview/",title:"Interview",pathLocale:"/",contents:[]},{path:"/GameEngine/Godot/",title:"Godot",pathLocale:"/",contents:[]},{path:"/GameEngine/",title:"Game Engine",pathLocale:"/",contents:[]},{path:"/GameEngine/Unreal/",title:"Unreal",pathLocale:"/",contents:[]},{path:"/Graphic/DirectX/",title:"Direct X",pathLocale:"/",contents:[]},{path:"/Graphic/basic/",title:"Basic",pathLocale:"/",contents:[]},{path:"/Graphic/bgfx/",title:"Bgfx",pathLocale:"/",contents:[]},{path:"/Platform/windows/",title:"Windows",pathLocale:"/",contents:[]},{path:"/Platform/",title:"Platform",pathLocale:"/",contents:[]},{path:"/Web/Project/",title:"Project",pathLocale:"/",contents:[]},{path:"/Basic/language/Golang/",title:"Golang",pathLocale:"/",contents:[]},{path:"/Basic/language/Java/",title:"Java",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/",title:"Unity",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/somecode/",title:"Somecode",pathLocale:"/",contents:[]},{path:"/GameEngine/Unreal/animation/",title:"Animation",pathLocale:"/",contents:[]},{path:"/GameEngine/Unreal/course/",title:"Course",pathLocale:"/",contents:[]},{path:"/GameEngine/Unreal/manual/",title:"Manual",pathLocale:"/",contents:[]},{path:"/GameEngine/Unreal/online/",title:"Online",pathLocale:"/",contents:[]},{path:"/GameEngine/Unreal/optimize/",title:"Optimize",pathLocale:"/",contents:[]},{path:"/Graphic/basic/geometry/",title:"Geometry",pathLocale:"/",contents:[]},{path:"/Graphic/basic/math/",title:"Math",pathLocale:"/",contents:[]},{path:"/Graphic/basic/shading/",title:"Shading",pathLocale:"/",contents:[]},{path:"/Graphic/basic/textureformat/",title:"Textureformat",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/manual/animation/",title:"Animation",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/manual/editor/",title:"Editor",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/manual/normal/",title:"Normal",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/manual/optimize/",title:"Optimize",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/manual/reader/",title:"Reader",pathLocale:"/",contents:[]},{path:"/GameEngine/Unity/manual/tools/",title:"Tools",pathLocale:"/",contents:[]},{path:"/category/",title:"Category",pathLocale:"/",contents:[]},{path:"/tag/",title:"Tag",pathLocale:"/",contents:[]},{path:"/article/",title:"Articles",pathLocale:"/",contents:[]},{path:"/star/",title:"Star",pathLocale:"/",contents:[]},{path:"/timeline/",title:"Timeline",pathLocale:"/",contents:[]}],q="update-vuepress-plugin-full-text-search2-search-index";var S=f(_),W=C(()=>{const e=new Map;for(const t of S.value)e.set(t.path,t);return e});import.meta.webpackHot&&(__VUE_HMR_RUNTIME__[q]=e=>{S.value=e});function H(e){const t=f([]);let s=null;return D(e,()=>{s&&clearTimeout(s),s=setTimeout(i,100)}),t;function i(){const c=e.value.toLowerCase().trim();if(!c){t.value=[];return}const o=new Map,a=new Set;for(const n of S.value)for(const r of z(n,c)){a.add(r.parentPageTitle);let l=o.get(r.parentPageTitle);l||(l=[],o.set(r.parentPageTitle,l)),l.push(r)}const d=[...a].sort((n,r)=>{const l=o.get(n);return o.get(r).length-l.length});t.value=[...o].flatMap(([,n])=>n).sort((n,r)=>n.parentPagePriority-r.parentPagePriority||d.indexOf(n.parentPageTitle)-d.indexOf(r.parentPageTitle)||n.priority-r.priority)}}function*z(e,t){const s=T(e.title,t);if(s){yield{path:e.path,parentPageTitle:A(e),title:e.title,display:s,page:e,content:null,parentPagePriority:1,priority:1};return}for(const i of e.contents){const c=T(i.header,t);if(c){yield{path:e.path+(i.slug?`#${i.slug}`:""),parentPageTitle:A(e),title:e.title,display:c,page:e,content:null,parentPagePriority:10,priority:2};continue}const o=T(i.content,t);o&&(yield{path:e.path+(i.slug?`#${i.slug}`:""),parentPageTitle:A(e),title:e.title,display:[{type:"header",str:`${i.header}
`},...o],page:e,content:null,parentPagePriority:10,priority:10})}}function A(e){const t=e.path.split("/");let s="/";return t[1]&&(s=`/${t[1]}/`),(W.value.get(s)||e).title}function T(e,t){const s=[];let i=0;const c=e.toLowerCase().replace(/\s/gu," ");let o=0,a=c.indexOf(t,o);if(a<0)return null;for(;a>=0;){const n=a+t.length;if(d(e.slice(o,a),"normal"),d(e.slice(a,n),"highlight"),o=n,a=c.indexOf(t,o),i>100)break}return d(e.slice(o),"normal"),s.filter(n=>n.str);function d(n,r){let l=n;r==="normal"&&l.length>100&&i===0&&(l=`… ${l.slice(-10)}`);let p=!1;if(i+l.length>100){if(s.some(g=>g.type==="ellipsis"))return;l=l.slice(0,Math.max(100-i,1)),p=!0}s.push({type:r,str:l}),i+=l.length,p&&(s.push({type:"ellipsis",str:" …"}),i+=2)}}var j={};const V=j,K=R({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>V}},setup(e){const{locales:t}=M(e),s=f(""),i=f(!1),c=f(-1),o=H(s),a=C(()=>s.value&&i.value&&o.value.length),d=L(),n=N(),r=C(()=>t.value[n.value]??{});function l(){if(!a.value)return;let h=c.value-1;h<0&&(h=o.value.length-1),g(h)}function p(){if(!a.value)return;let h=c.value+1;h>=o.value.length&&(h=0),g(h)}function g(h){c.value=h}function P(){c.value=-1}function x(h){if(!a.value)return;const k=o.value[h];k&&d.push(k.path)}return{query:s,focused:i,focusIndex:c,suggestions:o,activeSuggestion:a,onUp:l,onDown:p,focus:g,unfocus:P,go:x,locale:r}}}),Q={class:"search-box",role:"search"},Y=["placeholder"],X=["onMousedown","onMouseenter"],J=["href"],$={key:0,class:"parent-page-title"},Z={class:"suggestion-row"},ee={class:"page-title"},te={class:"suggestion-content"};function ae(e,t,s,i,c,o){return u(),m("div",Q,[F(y("input",{ref:"input","onUpdate:modelValue":t[0]||(t[0]=a=>e.query=a),"aria-label":"Search",class:b({focused:e.focused}),placeholder:e.locale.placeholder??"Search",autocomplete:"off",spellcheck:"false",onFocus:t[1]||(t[1]=()=>e.focused=!0),onBlur:t[2]||(t[2]=()=>e.focused=!1),onKeyup:[t[3]||(t[3]=w(a=>e.go(e.focusIndex),["enter"])),t[4]||(t[4]=w((...a)=>e.onUp&&e.onUp(...a),["up"])),t[5]||(t[5]=w((...a)=>e.onDown&&e.onDown(...a),["down"]))]},null,42,Y),[[U,e.query]]),e.activeSuggestion?(u(),m("ul",{key:0,class:"suggestions",onMouseleave:t[7]||(t[7]=(...a)=>e.unfocus&&e.unfocus(...a))},[(u(!0),m(G,null,I(e.suggestions,(a,d)=>(u(),m("li",{key:d,class:b(["suggestion",{focused:d===e.focusIndex}]),onMousedown:n=>e.go(d),onMouseenter:n=>e.focus(d)},[y("a",{href:a.path,onClick:t[6]||(t[6]=B(()=>{},["prevent"]))},[a.parentPageTitle&&(!e.suggestions[d-1]||e.suggestions[d-1].parentPageTitle!==a.parentPageTitle)?(u(),m("div",$,v(a.parentPageTitle),1)):E("v-if",!0),y("div",Z,[y("div",ee,v(a.title||a.path),1),y("div",te,[(u(!0),m(G,null,I(a.display,(n,r)=>(u(),m("span",{key:r,class:b(n.type)},v(n.str),3))),128))])])],8,J)],42,X))),128))],32)):E("v-if",!0)])}const oe=O(K,[["render",ae],["__scopeId","data-v-75f730df"],["__file","SearchBox.vue"]]);export{oe as default};
