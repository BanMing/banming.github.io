import{_ as n}from"./plugin-vue_export-helper-DlAUqK2U.js";import{e,f as a,o as l}from"./app-BeQLnLll.js";const i={};function c(p,s){return l(),e("div",null,s[0]||(s[0]=[a(`<h1 id="centos-7-根目录-lvm-扩容" tabindex="-1"><a class="header-anchor" href="#centos-7-根目录-lvm-扩容"><span>CentOS 7 根目录 LVM 扩容</span></a></h1><p>CentOS 默认用 LVM 管理磁盘,根目录满了不一定要重装。下面四种场景对应实际工作里最常碰到的几种情况:磁盘后面还有未分配空间、<code>/home</code> 占着空间但根目录不够、加了块新盘、在现有盘上新开分区。四种最后都落到同样三步:把空间塞进卷组 → <code>lvextend</code> 扩逻辑卷 → <code>xfs_growfs</code> 让文件系统认账。</p><p>开始前先 <code>lsblk</code> 看清楚现在的分区和卷组布局,<code>df -h</code> 看根目录还剩多少,别对着错的设备名敲命令。</p><h2 id="场景一-磁盘剩余空间扩到分区-再扩根目录" tabindex="-1"><a class="header-anchor" href="#场景一-磁盘剩余空间扩到分区-再扩根目录"><span>场景一:磁盘剩余空间扩到分区,再扩根目录</span></a></h2><p>适用于 <code>/dev/sda</code> 上分区 2 后面还有没分配的空间。</p><div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre><code><span class="line"><span class="token comment"># 查看磁盘情况</span></span>
